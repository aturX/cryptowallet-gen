...
const isMultiThreaded = !options.mnemonic || options.prefix || options.suffix || options['suffix-sensitive'] || options['prefix-sensitive'] || options.threads || isMainThread || workerData;
...
const availableThreads = cpuCount - 1; // leave 1 core for the main thread
const defaultThreads = Math.floor(cpuCount / 2); // use half of the available threads
const inputThreads = parseInt(options.threads || defaultThreads, 10);
const numThreads = inputThreads > availableThreads ? defaultThreads : inputThreads <= 0 ? availableThreads : inputThreads;
...
if (isMultiThreaded && isWalletGeneration) {
  if (isMainThread) {
    ...
    for (let i = 0; i < numThreads; i++) {
      ...
      // handle worker output and messages
      const handleWorkerOutput = (data) => {
